...
const SectionAccess = (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.sendForm('service_byy3rbf', 'template_cezuxnv', form.current, 'user_MmdiIBDzdFj4QZkkPVicV').then(
      (result) => {
        ...
        <section className='plans-container-slider'>
          <article className='plans-container-card'>
            <h3 className='plan-card-title'>
              <b>Beermoney Bot se encuentra en una fase de acceso semiprivado</b>
              <br />
              <small>
                {' '}
            ...
            <form className='plan-info-container' ref={form} onSubmit={handleSubmit} autoComplete='off'>
              ...
              <div className='captcha-container'>
                <ReCAPTCHA ref={captcha} sitekey='6Lc0ZFoeAAAAAPIksMFDS-02qLlFsHZ95bQKmDpu' onChange={onChange} />
              </div>
              <button className='plan-card-button' type='submit' disabled={captchaValid && !mySelect ? false : true}>
                {!isSubmitting ? <b>Enviar</b> : <i className='fas fa-spinner fa-spin'></i>}
              </button>
              <br />
              <br />
            </form>
